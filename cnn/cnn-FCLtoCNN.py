...
with g.as_default():
    ...
    conv1 = conv2d(input_tensor=input_layer,
                   ...
                   name='conv1')
    conv1=tf.nn.dropout(conv1, keep_prob=keep_proba)
    ...
    conv2=tf.nn.dropout(conv2, keep_prob=keep_proba)               
    ...
    flat = tf.reshape(pool2, shape=(-1, 1, 1, dims))

    out_layer=conv2d(input_tensor=flat, output_channels=n_classes, kernel_size=(1,1), strides=(1,1,1,1), activation=None, name='logits')
    out_layer=tf.reshape(out_layer, shape=(-1, n_classes))
